...
export const SettingsPage = (): React.JSX.Element => {
  const { settings, updateSettings, clearNews, clearActivity, resetLocalData, news, bookmarks, notes, sources } = useApp()
  ...
  const confirmCopy = confirmAction === 'news'
    ...
    : confirmAction === 'activity'
      ...
      : { title: 'Reset all local application data?', detail: 'This recreates the SQLite database, restores default settings and sources, removes synchronized news, bookmarks, and notes, and reimports the offline World English Bible.' }
  ...
  return <div className="page settings-page"><header className="page-header"><div><span className="eyebrow">Device-local configuration</span><h1>Settings</h1><p>Appearance, synchronization, Bible, timeline, region, accessibility, and privacy controls. No sign-in or remote preference account is used.</p></div><div className="privacy-chip"><Shield /><span><strong>Local-first privacy</strong><small>Preferences and personal research stay on this device.</small></span></div></header>
    <div className="settings-grid">
      <Panel title="Appearance" className="settings-card"><div className="theme-options"><button className={settings.theme === 'dark-gold' ? 'active' : ''} onClick={() => change('theme', 'dark-gold')}><Moon /><strong>Dark Gold</strong><small>Near-black surfaces with restrained warm gold.</small></button><button className={settings.theme === 'white-gold' ? 'active' : ''} onClick={() => change('theme', 'white-gold')}><Sun /><strong>White Gold</strong><small>Warm ivory surfaces with charcoal type.</small></button><button className={settings.theme === 'system' ? 'active' : ''} onClick={() => change('theme', 'system')}><Monitor /><strong>System Theme</strong><small>Follow the current Windows appearance.</small></button></div><label className="setting-row"><span><strong>Reduced motion</strong><small>Minimize transitions and animated feedback.</small></span><span className="switch"><input type="checkbox" checked={settings.reducedMotion} onChange={(event) => change('reducedMotion', event.target.checked)} /><i /></span></label></Panel>
      ...
      <Panel title="Bible & timeline" className="settings-card"><label className="setting-row"><span><strong>Default translation</strong><small>Only verified distributable translations are available.</small></span><select value={settings.defaultBible} disabled><option>WEB — World English Bible</option></select></label><label className="setting-row"><span><strong>Date display</strong><small>Historical events retain their own date-type metadata.</small></span><select value={settings.dateFormat} onChange={(event) => change('dateFormat', event.target.value === 'short' ? 'short' : 'long')}><option value="long">Long dates</option><option value="short">Short dates</option></select></label><div className="setting-stat"><Database /><span><strong>Complete offline WEB dataset</strong><small>Public domain text from eBible.org, imported into local SQLite.</small></span></div></Panel>
      ...
      <Panel title="About TruthNewsApp" className="settings-card about-settings"><div className="about-mark">T</div><div><h2>TruthNewsApp</h2><p>Truth. History. Prophecy. News.</p><dl><div><dt>Version</dt><dd>0.1.0 MVP</dd></div><div><dt>Platform</dt><dd>Windows desktop · Electron</dd></div><div><dt>Data model</dt><dd>Local-first SQLite</dd></div><div><dt>Bible</dt><dd>World English Bible · Public domain</dd></div><div><dt>Editorial direction</dt><dd>Christian · Biblical · Conservative</dd></div></dl></div></Panel>
    </div>
